class PackingLists::NewComponent < ApplicationComponent
  def initialize(road_trip:, packing_list:, current_user:)
    @road_trip = road_trip
  ...
  def view_template
    render ApplicationLayout.new(title: "New Packing List", current_user: @current_user) do
      div class: "max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8" do
        ...
        div class: "mb-8" do
          ...
          p class: "mt-2 text-sm text-gray-600" do
            "Create a packing list to organize items for your road trip."
          end
        end
          ...
          form_with model: [ @road_trip, @packing_list ], local: true, class: "space-y-6" do |form|
            div do
              ...
              form.text_field :name,
                              ...
                              placeholder: "e.g., Clothes, Electronics, Camping Gear...",
                              required: true

              if @packing_list.errors[:name].any?
                div class: "mt-1 text-sm text-red-600" do
                  @packing_list.errors[:name].first
                end
              end
            end

            # Visibility toggle
            render Shared::VisibilityToggleComponent.new(
              form: form,
              help_text: "Choose who can see this packing list. Private lists are only visible to you."
            )
            ...
            div class: "flex items-center justify-between pt-4" do
              ...
              form.submit "Create Packing List",
