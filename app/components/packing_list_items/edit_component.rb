class PackingListItems::EditComponent < ApplicationComponent
  ...
  def view_template
    render ApplicationLayout.new(title: "Edit #{@packing_list_item.name}", current_user: @current_user) do
      div class: "max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8" do
        ...
        div class: "mb-8" do
          ...
          p class: "mt-2 text-sm text-gray-600" do
            "Update the details of this packing list item."
          end
        end
          ...
          form_with model: [ @road_trip, @packing_list, @packing_list_item ], local: true, class: "space-y-6" do |form|
            ...
            div do
              div class: "flex items-center" do
                form.check_box :packed,
                               class: "h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                form.label :packed, class: "ml-2 block text-sm text-gray-700" do
                  "Already packed"
                end
              end

              if @packing_list_item.errors[:packed].any?
                div class: "mt-1 text-sm text-red-600" do
                  @packing_list_item.errors[:packed].first
                end
              end
            end
